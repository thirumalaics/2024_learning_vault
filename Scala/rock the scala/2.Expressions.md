...
- bitwise operators: &, |, ^,~(not), <<, >>, >>>(right shift with zero extension)
- equality check: 1 == x
- not equal: !=
- greater than: >
...
- instructions vs expressions
	...
	- expression: something that has a value and or a type
		...
		- ex: if expression
		- ```val aConditionedValue = if aCondition 5 else 3; println( aConditionedValue)```
...
- loops
	...
	- following is a scala 2 syntax with parantheses
```
...
```
var aVariable = 3
val aWeirdValue = (aVariable = 1) // aVariable's value had changed after this
println(aWeirdValue) // ()
```
...
- side effects in scala are expressions that returns a type unit
...
- the above piece of code is called code block
	...
	- even while loops, functions and classes can be nested within the code block